...
function matchRowHTML(j,showDate=false){ return `<div class="match-row"><div><div class="team">${safe(j.casa)}</div><div class="meta">${showDate ? `Jornada ${j.jornada} · ${fmtDate(j.data)} · ` : ''}${safe(j.hora || 'Hora a definir')}</div></div><div class="score">${scoreText(j)}</div><div><div class="team">${safe(j.fora)}</div><div class="meta">${safe(j.campo || 'Campo a definir')}</div></div></div>`; }
function mobileMatchHTML(j,showDate=false){ return `<div class="mobile-item"><div class="mobile-pair"><strong>${safe(j.casa)}</strong><span>${scoreText(j)}</span><strong>${safe(j.fora)}</strong></div><div class="meta">${showDate ? `Jornada ${j.jornada} · ${fmtDate(j.data)} · ` : ''}${safe(j.hora || 'Hora a definir')} · ${safe(j.campo || 'Campo a definir')}</div><span class="pill">${safe(j.estado || 'Agendado')}</span></div>`; }
...
function renderMatchMobile(container,jogos){ if(!jogos.length){container.innerHTML='<div class="mobile-item">Ainda não há jogos para apresentar.</div>';return;} container.innerHTML=jogos.map(j=>mobileMatchHTML(j,true)).join(''); }
function renderCalendario(desktop,mobile,jogos){
  ...
  const heading=(numero,data)=>`<header class="jornada-heading"><h2>Jornada ${numero}</h2><span>${fmtDate(data)}</span></header>`;
  desktop.innerHTML=[...jornadas].map(([numero,partidas])=>`<section class="jornada-card" id="jornada-${numero}">${heading(numero,partidas[0].data)}<div class="jornada-games">${partidas.map(j=>matchRowHTML(j)).join('')}</div></section>`).join('');
  mobile.innerHTML=[...jornadas].map(([numero,partidas])=>`<section class="jornada-card">${heading(numero,partidas[0].data)}<div class="jornada-mobile-games">${partidas.map(j=>mobileMatchHTML(j)).join('')}</div></section>`).join('');
}
...
async function initCalendario(){ try { const jogos=await loadJSON('jogos'); renderCalendario(document.querySelector('#calendario-list'),document.querySelector('#calendario-mobile'),jogos); } catch(error){ showLoadError(['#calendario-list','#calendario-mobile']); } }
