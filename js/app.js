...
function renderCalendario(desktop,mobile,jogos){
  ...
  desktop.innerHTML=[...jornadas].map(([numero,partidas])=>`<section class="jornada-card" id="jornada-${numero}">${heading(numero,partidas[0].data)}<div class="jornada-games">${partidas.map(matchRowHTML).join('')}</div></section>`).join('');
  mobile.innerHTML=[...jornadas].map(([numero,partidas])=>`<section class="jornada-card">${heading(numero,partidas[0].data)}<div class="jornada-mobile-games">${partidas.map(mobileMatchHTML).join('')}</div></section>`).join('');
}
